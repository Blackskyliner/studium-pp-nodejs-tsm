...
function TravellingSalesmanSolver(network, startNode)
{
    ...
    /**
     * Returns a partial problem and mark it as visited in our tree.
     *
     * @TODO Seperate easier problems -> expand our _v to also hold multiple depths or create new variable to hold future visited...
     *
     * @param optimalDepth Best depth which will be solved in acceptable time (generated through external heuristic)
     * @param [limit] How far should we go up the tree to find a harder problem to separate
     */
    function getPartialProblem(optimalDepth, limit)
    {
        if(limit === undefined) limit = 1;
        if(optimalDepth <= 1){ limit = 0; optimalDepth = 1; }

        var s  = _.clone(_s); // just to be sure we won't work on _s and interfere with solve...
        var ts = _.clone(_s); // holds a truncated path

        // We can only separate problems on our path s, because we only hold its visited nodes and no more! (see TODO)

        if(s.length > optimalDepth)
        { // Goto our optimalDepth if we ware not already there
            var subPath = [];
            while(subPath.length != optimalDepth) subPath.push(s[subPath.length]);
            s = subPath;
        }

        // Truncate out path so we know where we are in our solve, so we won't split the path.
        while(s.length + 1 != ts.length) ts.pop();

        while(s.length >= optimalDepth - limit)
        {
            var u = getUnvisitedNodes(s);

            while(u.length)
            {
                var childNode = u.pop(); // get the last problem in out unvisited array, reduce with solve as it uses shift()
                if(!_.contains(ts, childNode))
                { // if it is not on our real current (solve)path
                    _v[s.length - 1].push(childNode); // Set it to visited
                    s.push(childNode); // push it on our partial problem path
                    return exports.Util.nodePathToNamePath(s); // and return the partial problem - yay!
                }
            }

            s.pop(); // pop the current path (every pop() will make the partial problem harder to solve (see limit))
        }

        return null; // No problem (inside our difficult range) separable try again later...
    }
     ...
     *
     * @param [maxSteps] Just does x Steps and returns.
     * @param [callback] This callback will be called on each found (best) path
     */
    function solve(maxSteps, callback)
    {
        if(_debug){
        ...
        while(_s.length > 0)
        {
            if(!_.isUndefined(maxSteps) && maxSteps-- <= 0) break;
    ...
    return {
        getNetworkSize: function(){ return network.length; },
        isFinished: function(){
            return _s.length === 0;
        },
        getPartialProblem: getPartialProblem
        ,setPartialProblem: setPartialProblem
        ,setStartNode: setStartNode
        ,solve: solve,
        setBestCost: function(path, cost) { _bestCost.cost = cost; _bestCost.path = path; }
        /**
